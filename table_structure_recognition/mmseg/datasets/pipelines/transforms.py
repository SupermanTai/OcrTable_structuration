...
class Resize(object):
    ...
    def imrescale(img,
                  ...
                  padding=False,
                  pad_val=0):
        """todo Resize image to fixed size with unchanged aspect ratio using padding
        ...
        """
        from mmcv import rescale_size, imresize
        h, w = img.shape[:2]
        ...
        if padding:
            ...
            nw, nh = new_size[0], new_size[1]
            boxed_image[(h-nh)//2:(h-nh)//2+nh, (w-nw)//2:(w-nw)//2+nw, :] = rescaled_img
            rescaled_img = boxed_image
